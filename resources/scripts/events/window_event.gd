...
func _on_animation_player_animation_finished(_window_open):
	print_debug(_window_open + " animation completed.")
	if Has_Window_Opened == 0:
		print_debug("Window was closed in time with a status of " + str(Has_Window_Opened) + ". Continuing game.")
		SignalManager.reset_monster.emit()
		pass
	else:
		print_debug("Window opened with a status of " + str(Has_Window_Opened) + ". Game over.")
		SignalManager.game_over.emit()
		pass

func _on_animation_player_animation_started(_window_open):
	#print_debug(_window_open + " animation started.")
	pass
